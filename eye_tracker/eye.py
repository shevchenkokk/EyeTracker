...
class Eye:
    '''
    Класс для представления глаза на изображении с лицом, содержащий
    методы для анализа и определения характеристик глаза
    '''
    ...
    def __init__(self, frame, face_shape, is_left_eye):
        ...
        # набор точек, представляющих форму лица
        self.face_shape = face_shape
        
        # соответствует ли экземпляр класса левому глазу
    ...
    def _find_dist(vec1, vec2):
        # определение расстояния между ключевыми точками
        return math.sqrt(
             math.pow(vec1[0] - vec2[0], 2) + math.pow(vec1[1] - vec2[1], 2)
        )

    def _get_eye_aspect(self):
        # вычисление отношения между шириной и высотой глаза
    ...
    def _get_gaze_relation(self):
        # создание маски изображения размером с кадр
        mask = np.zeros(self.frame.shape[:2], dtype=np.uint8)
        # отрисовка и заполнение внутренностей контуров глаза на маске
        ...
        # фильтрация изображения для сглаживания шумов
        ...
        # разделение изображения на области, соответствующие левой и правой части глаза
        left_side_threshold = threshold[0: height, 0: int(width / 2)]
        right_side_threshold = threshold[0: height, int(width / 2): width]
        
        #подсчёт количества белых пикселей на левой и правой части глаза
